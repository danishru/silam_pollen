...
- 🌸 **Сенсор прогноза всегда создаётся**  
  `weather.silam_pollen_*_forecast` появляется при любой конфигурации — данные *now* доступны всегда, а **почасовой**, **двухразовый (каждые 12 ч)** и **суточный** прогнозы показываются **только если включена опция прогноза**.
...
- ⏱️ **Диагностический Forecast Horizon**  
  Новый `sensor.silam_pollen_forecast_horizon` показывает, сколько часов охватывает актуальный прогноз (`state`) и какую длительность (`forecast_duration`) вы запросили.

- 🖼️ **Поддержка в pollenprognos-card v2.3.5+**  
  Карточка теперь отображает ежедневный прогноз через `weather.get_forecasts`.  
  > **Совместимость версий**  
  > • *pollenprognos-card* ≥ **v2.3.5** требует **silam_pollen ≥ v0.2.7**  
  > • Старые версии карточки (≤ v2.3.4) остаются совместимыми с silam_pollen ≥ v0.2.5.
