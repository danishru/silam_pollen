...
# Минимальное покрытие вперёд (в часах) для текущего датасета.
# Если runs/catalog.xml сообщает, что данных нет хотя бы до now+N часов,
# SMART может переоценить и переключить базовый датасет.
SMART_MIN_COVERAGE_HOURS = 1
SMART_NCSS_FAIL_STREAK_SWITCH = 2
